...
public class TodoListPage extends Composite {
	...
	@Inject
	private Caller<TodoListService> todoListService;
	...
	@Inject
	private @DataField
	ListWidget<SharedList, SharedListWidget> sharedContainer;
	...
	@Inject
	private TransitionTo<LoginPage> logoutTransition;
	@Inject
	private TransitionTo<SharePage> sharePageTransition;
	@Inject
	private @DataField
	Anchor logoutLink;
	...
	private void refreshItems() {
		...
		TypedQuery<TodoItem> query = em.createNamedQuery("currentItemsForUser",
				TodoItem.class);
	...
	void onNewItem(KeyDownEvent event) {
		if (event.getNativeKeyCode() == KeyCodes.KEY_ENTER
				&& !newItemBox.getText().trim().equals("")) {
