...
public class SignupPage extends Composite {
	...
	@Inject
	private TransitionTo<TodoListPage> todoListPageLink;
	...
	private void doSignup(ClickEvent e) {
		Set<ConstraintViolation<TodoListUser>> violations = validator
				.validate(user);
		if (violations.size() > 0) {
			ConstraintViolation<TodoListUser> violation = violations.iterator()
					.next();
			overallErrorMessage.setText(violation.getPropertyPath() + " "
					+ violation.getMessage());
			overallErrorMessage.setVisible(true);
			return;
		}

		// Passwords are not part of the user bean, so we do manual validation
		// for these fields
